...
export function getTimezoneOffset(timeZone: string, prefix: "GMT" | "UTC" = "GMT", padded: boolean = false): string {
  const date = new Date();
...
export function getLocalTime(timeZone: string): { time: string; offset: string } {
  return {
