  ...
  const stats: { icon: React.ReactNode; label: React.ReactNode; sublabel: React.ReactNode; ready: boolean }[] = [
    {
      ...
      label: <><AnimateText words={[age]} variant="slot" cursor="none" className="text-sm leading-5" /> years old</>,
          ...
          {stats.map((stat, i) => (
            <li key={i} className="break-inside-avoid mb-6 flex items-start gap-4 text-zinc-600 dark:text-zinc-400">
              ...
              <div className="flex min-w-0 flex-col gap-1">
                {stat.ready ? (
                  <span className="leading-5 truncate text-sm text-zinc-800 dark:text-zinc-200">{stat.label}</span>
                ) : (
